...
def main():
    parser = argparse.ArgumentParser(description="Deploy Izumi Agent to Vertex AI Agent Platform")
    parser.add_argument(
        "--project",
        default=None,
    ...
    if not project_id:
        print("❌ Error: Active GCP Project ID not specified. Set GOOGLE_CLOUD_PROJECT or pass --project.")
        sys.exit(1)

    if not staging_bucket:
        # Derive default bucket name standard
        staging_bucket = f"{project_id}-agent-engine"
        print(f"ℹ️ ASSET_SERVICE_GCS_BUCKET not specified. Defaulting to: {staging_bucket}")
    ...
    try:
        for item in os.listdir(backend_src_path):
            ...
            if item in [".venv", "__pycache__", ".pytest_cache", "dist", "deployment_metadata.json"]:
                continue
            if os.path.isdir(s):
                shutil.copytree(s, d)
            else:
        ...
        env_vars = {
            ...
            "CREATIVE_STUDIO_FRONTEND_URL": os.getenv("CREATIVE_STUDIO_FRONTEND_URL", ""),
            "CREATIVE_STUDIO_USER_AUTH_TOKEN_KEY": os.getenv("CREATIVE_STUDIO_USER_AUTH_TOKEN_KEY", ""),
        ...
        extra_packages = ["ads_x", "mediagent_kit", "utils"]
        ...
        existing_engines = list(agent_engines.list(filter=f'display_name="{args.agent_name}"'))
        if existing_engines:
            print(f"🔄 Updating existing reasoning engine instance: {args.agent_name} ({existing_engines[0].resource_name})")
        ...
        print(f"• Console Link:  https://console.cloud.google.com/vertex-ai/publishers/google/model-garden/agent-engine?project={project_id}")
        print("="*60 + "\n")
        ...
        if os.path.exists(env_file):
            ...
            if "USE_AGENT_ENGINE=" in env_content:
                env_content = re.sub(r"USE_AGENT_ENGINE=\w+", "USE_AGENT_ENGINE=True", env_content)
            else:
                env_content += "\nUSE_AGENT_ENGINE=True"

            if "AGENT_ENGINE_ID=" in env_content:
                env_content = re.sub(r"AGENT_ENGINE_ID=[^\n]*", f"AGENT_ENGINE_ID={engine_id}", env_content)
            else:
