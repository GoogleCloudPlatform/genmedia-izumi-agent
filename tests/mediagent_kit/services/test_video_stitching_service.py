...
from mediagent_kit.services.types import VideoTimeline, VideoClip, AudioClip, Transition, TransitionType, Asset, Trim, AudioPlacement
...
def test_build_filter_complex_single_video(mock_asset_service, mock_config):
    ...
    service = VideoStitchingService(asset_service=mock_asset_service, config=mock_config)
...
def test_create_placeholder_clip(mock_get_ffmpeg, mock_subprocess_run, mock_asset_service, mock_config):
    from mediagent_kit.services.video_stitching_service import VideoStitchingService
...
def test_stitch_video_success(
    mock_get_ffmpeg, mock_subprocess_run, mock_get_metadata, mock_asset_service, mock_config
):
    from mediagent_kit.services.video_stitching_service import VideoStitchingService
    ...
    timeline = VideoTimeline(
        ...
        video_clips=[
            VideoClip(asset=mock_source_asset)
        ],
        transitions=[],
        audio_clips=[]
    )
    
    result = service.stitch_video(user_id="user_123", timeline=timeline, output_filename="result.mp4")
...
def test_stitch_video_with_image_clip(
    ...
    timeline = VideoTimeline(
        title="Image Video",
        video_clips=[
            VideoClip(asset=mock_source_asset, trim=Trim(duration_seconds=5))
        ],
        transitions=[],
        audio_clips=[]
    )
...
def test_stitch_video_with_placeholder_clip(
    ...
    timeline = VideoTimeline(
        ...
        video_clips=[
            VideoClip(asset=None, placeholder="Loading...", trim=Trim(duration_seconds=5))
        ],
        transitions=[],
        audio_clips=[]
    )
...
def test_stitch_video_with_audio_clip(
    ...
    timeline = VideoTimeline(
        ...
        transitions=[],
        audio_clips=[
            AudioClip(asset=mock_audio_asset, start_at=AudioPlacement(video_clip_index=0, offset_seconds=0))
        ]
    )
