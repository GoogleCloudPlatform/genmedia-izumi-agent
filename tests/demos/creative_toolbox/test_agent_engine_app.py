...
import pytest
from demos.backend.creative_toolbox.agent_engine_app import AgentEngineApp, deploy_agent_engine_app


class TestAgentEngineAppClass(unittest.TestCase):
    def test_set_up(self):
        with patch("demos.backend.creative_toolbox.agent_engine_app.TracerProvider") as mock_tp:
            with patch("demos.backend.creative_toolbox.agent_engine_app.export.BatchSpanProcessor") as mock_bsp:
                with patch("demos.backend.creative_toolbox.agent_engine_app.CloudTraceLoggingSpanExporter") as mock_exporter:
                    with patch("demos.backend.creative_toolbox.agent_engine_app.trace.set_tracer_provider") as mock_set_tp:
                        ...
                        with patch("vertexai.preview.reasoning_engines.AdkApp.set_up") as mock_super_set_up:
                            app.set_up()
    ...
    def test_register_feedback(self):
        ...
        with patch("demos.backend.creative_toolbox.agent_engine_app.Feedback") as mock_feedback_cls:
    ...
    def test_register_operations(self):
        app = AgentEngineApp(agent=MagicMock())
        with patch("vertexai.preview.reasoning_engines.AdkApp.register_operations") as mock_super_reg:
            ...
            with patch("vertexai.agent_engines.list") as mock_list:
                with patch("vertexai.agent_engines.create") as mock_create:
                    with patch("vertexai.agent_engines.update") as mock_update:
                        ...
                        mock_remote.resource_name = "projects/p/locations/l/agentEngines/a"
                        mock_create.return_value = mock_remote
                        
                        # Mock datetime.datetime.now
                        with patch("datetime.datetime") as mock_datetime:
                            mock_datetime.now.return_value.isoformat.return_value = "2026-04-01T00:00:00"
