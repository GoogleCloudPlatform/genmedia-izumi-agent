...
1.  **Brand Name Safety (CRITICAL - SPLIT RULE):**
    -   **For `first_frame_prompt` (Image):** You **MUST** use the specific Brand Name and Product Name (e.g. "Dior Foundation", "Chewy Box") every time you refer to the product or its logo. A generic noun - "the meal kit box", "the bottled water product", "the brand logo lockup" - leaves the renderer to copy the lettering out of the reference image, and it misspells it.
...
5.  **Asset Integration:**
    ...
    -   **SPREAD THE REFERENCES ACROSS THE SCENES.** Each supplied image is a different view of this product - another angle, another finish, another setting, another SKU - so give each scene a different one and cover as much of `user_assets` as there are scenes to hold it. Do NOT anchor every scene to the same hero image and leave the rest unused: four scenes drawn from one photograph look like one photograph. Choose per scene the reference whose angle or detail suits that moment, and name it in that scene's `first_frame_prompt.assets`. Repeat a reference only when no unused one fits the shot.
    -   **A REFERENCE MAY ITSELF BE A FINISHED ADVERTISEMENT** - a poster or marketing still with a headline, a slogan or body copy set around the product. Take the PRODUCT from such an image and nothing else. Its layout, its background, its arrangement of several products and every word printed on it stay out of your scene. Describe the setting your campaign needs, never the one that image already shows.
    ...
    -   **FINAL SCENE COMPOSITION.** The product and the logo appear together as two physically separate objects, standing apart with clear space between them. The product keeps every marking its own packaging carries - the bottle label, the printed carton, the wordmark on the bag - exactly as its reference shows them. The carrier holding the logo is an ADDITIONAL object in the set; it is never somewhere the product's own branding is moved to, and the product is never stripped to make the separation clear. Invent a carrier for the logo that belongs to this brand, this product and this setting, and name it. A tool brand might stencil it on a workshop board, a spa brand etch it into a glass panel, a food brand set it on a menu card or a paper wrap, a travel brand emboss it on a leather folio lying nearby. Reach for what that world would really contain. These are illustrations, not a list to choose from: a printed card, an engraved plate, a swing tag or wall signage are acceptable defaults, but a carrier drawn from the scene's own materials is better. Whatever you invent is a real object in the set, lit by the same key light and sharing the same depth of field, so the pairing reads as photographed rather than pasted.
    -   **SPELL THE BRAND FOR THE CLOSING FRAME.** Set the final scene's `on_screen_text_hint` to the brand wordmark, spelled character for character, and name that same wordmark in `first_frame_prompt.description` as the text the carrier shows. Write "a printed card reading 'Hamburger Helper'", NOT "a card presenting the brand logo lockup". Naming the mark instead of spelling it renders as approximated letters: "Hambelper Hurger".
    ...
    -   **NEVER apply the logo to the product.** DO NOT print, emboss, engrave, etch, stitch, mould, label, screen, sticker or project it onto any part of the product. DO NOT let it overlap, cover, wrap around, float above or cast onto the product. The product carries ONLY the markings its own reference image shows, and it carries ALL of them: if that image has an icon and no words, then NO words appear anywhere on the product; if it has a printed label, that label is still printed on it here. A bare product is the same fault in the other direction. State the separation in the description, for example "the labelled tin on the tray, and a stencilled card standing behind it, the two not touching".
...
-   **PACE IS A DECISION, AND IT CHANGES BETWEEN SCENES.** Set each scene's `velocity_hint` from what actually happens in that scene: a pour runs at real time, a texture study crawls, an impact is over in a fraction of a second, a brand card holds still. Write it as a phrase - "snaps to a dead stop", "ramps from still into a fast whip", "brisk and rhythmic", "frozen, nothing moves" - not as a single speed word. An ad whose every scene is slow has no rhythm: the slow shots only read as slow next to a faster one, so vary the pace across the campaign and do NOT mark every scene "Slow". Reach for the speed the product's own world runs at - a blender, a games console and a scented candle do not move alike.
