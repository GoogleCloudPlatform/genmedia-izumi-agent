...
async def stitch_final_video(tool_context: ToolContext) -> ToolResult:
    ...
    logger.error("⭐⭐⭐ [NATIVE TOOL INVOCATION] `stitch_final_video` WAS SUCCESSFULLY TRIGGERED ⭐⭐⭐")
    ...
    if not scenes:
        return tool_failure("Malformed storyboard: 'scenes' key is missing. Cannot stitch without scenes.")
    ...
    for index, scene in enumerate(scenes):
        ...
        if not first_frame_prompt.get("asset_id"):
            logger.warning(f"Skipping scene {index} in final stitch because it has no asset_id (generation likely failed).")
            continue
    ...
    if voiceover_groups and not is_ugc:
        ...
        for group in voiceover_groups:
            ...
            if group.get("audio_asset_id"):
                voiceover_asset = await asset_service.get_asset_by_id(group["audio_asset_id"])
                if voiceover_asset:
                    speed = 1.0
                    ...
                    if target_clip_idx is not None:
                        audio_clips.append(
                            types.AudioClip(
                                ...
                                start_at=types.AudioPlacement(video_clip_index=target_clip_idx),
                                speed=speed,
                            )
                        )
                    else:
                        logger.warning(f"Skipping grouped voiceover for scene {group['scene_indices'][0]} because it was not included in the stitch.")
            ...
            if voiceover_prompt.get("asset_id"):
                ...
                if voiceover_asset:
                    ...
                    if target_clip_idx is not None:
                        ...
                        logger.warning(f"Skipping per-scene voiceover for scene {index} because it was not included in the stitch.")
            else:
                # UGC / Lip-Sync Logic: Add the video's own audio track
                video_asset_id = video_prompt.get("asset_id")
                if video_asset_id:
                    video_asset = await asset_service.get_asset_by_id(video_asset_id)
                    if video_asset.current.video_generate_config and video_asset.current.video_generate_config.generate_audio:
                        target_clip_idx = scene_to_clip_index.get(index)
                        if target_clip_idx is not None:
                            audio_clips.append(
                                types.AudioClip(
    ...
    display_result = await display_asset(tool_context=tool_context, asset_id=stitched_asset.id)
