...
from ...instructions.parameters import parameters_instruction, parameters_repair_instruction

logger = logging.getLogger(__name__)

async def extract_campaign_parameters(tool_context: ToolContext, user_brief: str) -> str:
    """Parses the user brief into structured campaign parameters silently in the background."""
    
    logger.error("⭐⭐⭐ [NATIVE TOOL INVOCATION] `extract_campaign_parameters` WAS SUCCESSFULLY TRIGGERED ⭐⭐⭐")
    logger.error(f"⭐⭐⭐ [NATIVE TOOL CONTEXT] Received User Brief Length: {len(user_brief)} characters ⭐⭐⭐")
    ...
    extraction_result = await mediagen_service.generate_text_with_gemini(
        ...
        prompt=parameters_instruction.INSTRUCTION + f"\n\n**USER BRIEF:**\n{user_brief}",
    ...
    try:
        ...
        logger.warning(f"🚨 [DEEP DEBUG UPSTREAM] Persisting to state type: {type(dumped)}")
        logger.warning(f"🚨 [DEEP DEBUG UPSTREAM] Persisting to state repr: {repr(dumped)}")
        logger.info(f"Successfully persisted parameters to state: {params.campaign_name}")
        
        return "Campaign parameters extracted successfully."
    except Exception as e:
        first_error = e
        logger.warning(f"First parameter extraction failed: {e}. Attempting self-correction...")
    ...
    try:
        ...
        repair_result = await mediagen_service.generate_text_with_gemini(
            ...
            model="gemini-2.5-flash",
            prompt=parameters_repair_instruction.REPAIR_PROMPT.format(user_brief=user_brief, raw_json=clean_json, error=str(first_error)),
            reference_image_filenames=[],
        )
        ...
        logger.info(f"Successfully persisted repaired parameters to state: {params.campaign_name}")
        
        return "Campaign parameters extracted (Repaired)."
    except Exception as e2:
        logger.error(f"Parameter repair also failed: {e2}. Falling back to intelligent guessing.")
        ...
        if detected_template == "Custom":
            suggested = template_library.suggest_template(industry="General", vertical=user_brief[:50])
        ...
        logger.info(f"Successfully persisted fallback parameters to state: {fallback_params.campaign_name}")
