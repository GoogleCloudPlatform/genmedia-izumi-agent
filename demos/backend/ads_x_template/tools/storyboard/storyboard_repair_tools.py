...
async def finalize_and_persist_storyboard(
    ...
    logger.error("⭐⭐⭐ [NATIVE TOOL INVOCATION] `finalize_and_persist_storyboard` WAS SUCCESSFULLY TRIGGERED ⭐⭐⭐")
    
    logger.info(f"finalize_and_persist_storyboard received raw_output of length: {len(raw_output)}")
    ...
    try:
        ...
        logger.warning(f"Storyboard JSON is malformed or truncated: {e}. Attempting repair...")
        ...
        repair_result = await mediagen_service.generate_text_with_gemini(
            ...
            prompt=REPAIR_PROMPT.format(raw_json=clean_json[-5000:]), # Send last 5k chars for context
        ...
        repair_blob = await mediagent_kit.services.aio.get_asset_service().get_asset_blob(repair_result.id)
    ...
    try:
        ...
        if template_name == "Custom" and storyboard.scenes:
            ...
            valid_counts = pacing_blueprints.get_valid_scene_counts_for_duration(target_secs)
            if scene_cnt not in valid_counts:
                logger.error(f"Pacing Constraint Violation: Generated {scene_cnt} scenes for {target_secs}s. Requires: {valid_counts}")
                raise ValueError(f"Invalid scene count. For a {target_secs}s video, you MUST generate exactly one of the following scene counts: {valid_counts} scenes. You generated {scene_cnt} scenes. Please rewrite the entire storyboard JSON to match the required scene count.")
            ...
            if pacing_blueprints.matches_any_preset(target_secs, llm_durations):
                logger.info(f"LLM perfectly matched a valid pacing preset for {target_secs}s. Proceeding without override.")
            else:
                logger.warning(f"LLM pacing {llm_durations} invalid for target {target_secs}s. Enforcing blueprint override.")
                blueprint_durations = pacing_blueprints.get_blueprint_for_count(target_secs, len(storyboard.scenes))
        ...
        for i, scene in enumerate(storyboard.scenes, 1):
            table_rows.append(f"| {i} | {scene.video_prompt.description} | *\"{scene.voiceover_prompt.text}\"* |")
...
"""
        ...
        return tool_failure(f"Storyboard validation failed. If the 'scenes' array is missing or empty, you MUST rethink and completely regenerate the JSON object, explicitly including all required scenes. Error details: {ve}")
