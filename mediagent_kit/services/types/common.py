...
AssetStatus = Literal["completed", "failed", "pending"]
...
class GeneratedAsset(BaseModel):
    ...
    ``status`` can take any of {``"completed"``, ``"failed"``,
    ``"pending"``} — Creative Studio's backend genuinely produces
    pending assets (e.g. an asset row created the moment a generation
    is enqueued, before the model finishes), and any ``get_asset`` /
    ``search_assets`` call may return one in that state.

    However, **the return values of ``generate_*`` and
    ``stitch_timeline`` are terminal-only by contract** — they MUST
    return a ``GeneratedAsset`` with ``status`` in
    {``"completed"``, ``"failed"``}, never ``"pending"``. Async
    backends are responsible for polling internally before returning.
    The type system allows ``"pending"`` so it can appear on
    list/get paths; the contract narrows it out at the generation
    boundary. See ``MediaGenerationServiceInterface`` docstring.
    """
