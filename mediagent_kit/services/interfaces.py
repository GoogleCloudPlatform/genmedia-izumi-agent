...
class MediaGenerationServiceInterface(ABC):
    ...
    Contract: all ``generate_*`` methods MUST return a TERMINAL
    ``GeneratedAsset`` — ``status`` is ``"completed"`` or
    ``"failed"``, never ``"pending"``. Backends that generate
    asynchronously are responsible for polling internally before
    returning. This keeps the abstraction honest across blocking
    (Izumi Native) and poll-based (Creative Studio) backends.
    """
    ...
    async def generate_text(
        ...
        """Generates text. Stored as a ``GeneratedAsset`` (text inline
        in ``gcs_uri`` as a ``data:`` URI for small payloads, or in
        GCS for larger).

        Returning ``GeneratedAsset`` rather than a bare ``str`` keeps
        the interface symmetric with the other generators and lets
        callers fetch / audit / replay the generation later.
        """
