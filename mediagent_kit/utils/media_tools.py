...
def strip_audio_from_video_blob(blob: bytes, file_extension: str = "mp4") -> bytes:
    """Removes the audio track from a video, leaving the video stream untouched.

    Some generators mux audio into every clip with no way to ask them not to.
    This pipeline builds its own soundtrack - voiceover timed to the script,
    a single music bed across the whole cut - and stitches it on at the end,
    so a per-clip track generated by the model would play underneath all of
    that. The video is copied rather than re-encoded, so nothing is lost.
    """
    if not file_extension.startswith("."):
        file_extension = f".{file_extension}"
    ...
    try:
        ...
        if result.returncode != 0:
            logger.error(f"ffmpeg failed to strip audio: {result.stderr}")
            raise RuntimeError("Failed to strip the audio track from the video.")
